...
namespace PieceManager
{
    ...
    {
        public readonly List<Requirements> Requirements = new();

        public void Add(string item, int amount, int amountPerLevel, bool recover) => Requirements.Add(new Requirements
            { item = item, amount = amount, amountPerLevel = amountPerLevel, recover = recover });
    }
    ...
    public class BuildPiece
    {
        ...
        private static Dictionary<BuildPiece, List<Piece.Requirement>> activeRecipes = new();
            ...
            if (ConfigurationEnabled)
            {
                foreach (BuildPiece? piece in registeredPieces)
                {
                    ...
                    string localizedName = new Regex("['[\"\\]]")
                        .Replace(english.Localize(piece.Prefab.GetComponent<Piece>().m_name), "").Trim();
                    ...
                    ConfigEntry<string> itemConfig(string name, string value, string desc)
                    {
                        ...
                        return config(localizedName, name, value, new ConfigDescription(desc, null));
                    ...
                    {
                        if (ObjectDB.instance && activeRecipes.ContainsKey(piece))
                        {
                            piece.Prefab.GetComponent<Piece>().m_resources = SerializedRequirements.toPieceReqs(
        ...
        internal static void Patch_PieceAwake(ref Piece __instance)
        {
            if (__instance == null)
            {
                return;
        ...
        {
            public readonly List<Requirements> Reqs;

            public SerializedRequirements(List<Requirements> reqs) => Reqs = reqs;
            ...
            public SerializedRequirements(string reqs)
            {
                Reqs = reqs.Split(',').Select(r =>
                {
                    ...
                    return new Requirements
                    {
                        ...
                        amountPerLevel = parts.Length > 1 && int.TryParse(parts[2], out int amountPerLevel)
                            ? amountPerLevel
                            : 1,
                        recover = parts.Length <= 1 || !bool.TryParse(parts[3], out bool recover) || recover,
                    };
                }).ToList();
            }
            ...
            {
                return string.Join(",", Reqs.Select(r => $"{r.item}:{r.amount}:{r.amountPerLevel}:{r.recover}"));
            }

            public static Piece.Requirement[] toPieceReqs(SerializedRequirements craft)
            {
                ItemDrop? ResItem(Requirements r)
                {
                    ItemDrop? item = ObjectDB.instance.GetItemPrefab(r.item)?.GetComponent<ItemDrop>();
                    if (item == null)
                    {
                        Debug.LogWarning($"The required item '{r.item}' does not exist.");
                    }

                    return item;
                }

                Dictionary<string, Piece.Requirement?> resources = craft.Reqs.Where(r => r.item != "")
                    .ToDictionary(r => r.item,
                        r => ResItem(r) is { } item
                            ? new Piece.Requirement
                            {
                                m_amount = r.amount, m_resItem = item, m_amountPerLevel = r.amountPerLevel,
                                m_recover = r.recover
                            }
                            : null);
    ...
    public static class PiecePrefabManager
    {
        ...
        static PiecePrefabManager()
        {
            ...
            harmony.Patch(AccessTools.DeclaredMethod(typeof(ObjectDB), nameof(ObjectDB.Awake)),
                new HarmonyMethod(AccessTools.DeclaredMethod(typeof(PiecePrefabManager), nameof(Patch_ObjectDBInit))));
        ...
        {
            foreach (GameObject prefab in prefabs.Where(prefab => !__instance.m_items.Contains(prefab)))
            {
        ...
        private static void Patch_ZNetSceneAwakePost(ZNetScene __instance)
        {
            ...
            foreach (GameObject prefab in prefabs.Concat(ZnetOnlyPrefabs))
            {
                ...
                if (_hammer is not null && _hammer.m_pieces.Contains(prefab)) return;
                _hammer?.m_pieces.Add(prefab);
            }
        }
    }
}